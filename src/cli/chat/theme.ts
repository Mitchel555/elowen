...
export const color = {
  ...
  // A concrete near-black RGB foreground, NOT ANSI `30;1` (black+bold): xterm.js with the default
  // drawBoldTextInBrightColors renders bold black as brightBlack (#666), which on a bright accent
  // selection background drops to ~1.8:1 — an unreadable selected row. Every preset selectedBg is a
  // bright accent, so this dark text clears 4.5:1 on all of them (≥5.7:1).
  selected: (s: string): string => sgr(`${activeTheme.selectedBg};${fg(20, 18, 24)}`, s),
};
