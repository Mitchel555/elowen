...
describe('appIdentity', () => {
  it('defaults to the public elowen.run referer + "Elowen" title', async () => {
    const m = await loadFresh({ ELOWEN_APP_URL: undefined, ELOWEN_APP_TITLE: undefined });
    expect(m.APP_URL).toBe('https://elowen.run');
    ...
    expect(m.APP_IDENTITY_HEADERS['http-referer']).toBe('https://elowen.run');
