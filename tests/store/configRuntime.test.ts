...
describe('ConfigStore runtime limits', () => {
  ...
  it('accepts every knob at both edges of its range', () => {
    const cs = new ConfigStore(openDb(':memory:'));
    cs.update({ runtime: { limits: { localShellTimeoutMs: 10_000, memorySemanticFloorPerMille: 100, toolDeferThreshold: 1, eventRetentionDays: 1 } } });
    ...
    cs.update({ runtime: { limits: { localShellTimeoutMs: 300_000, memorySemanticFloorPerMille: 800, toolDeferThreshold: 100, eventRetentionDays: 365 } } });
    expect(cs.get().runtime.limits).toEqual({
  ...
  it('clamps a value past either end back into range', () => {
    const cs = new ConfigStore(openDb(':memory:'));
    cs.update({ runtime: { limits: { localShellTimeoutMs: 1, memorySemanticFloorPerMille: 0, toolDeferThreshold: 0, eventRetentionDays: 0 } } });
    expect(cs.get().runtime.limits).toEqual({
      localShellTimeoutMs: 10_000, memorySemanticFloorPerMille: 100, toolDeferThreshold: 1, eventRetentionDays: 1,
    });
    cs.update({ runtime: { limits: { localShellTimeoutMs: 9_000_000, memorySemanticFloorPerMille: 1_000, toolDeferThreshold: 5_000, eventRetentionDays: 10_000 } } });
    expect(cs.get().runtime.limits).toEqual({
      localShellTimeoutMs: 300_000, memorySemanticFloorPerMille: 800, toolDeferThreshold: 100, eventRetentionDays: 365,
    });
  });
